  ...
  def handle_noargs(self, **options):
    for host in Host.objects.all():
      ...
      for app in host.apps.all():
        ...
        appMenu = AppMenu(app = app, title = app.title, description = app.description, parentItem = None, host = host)
        appMenu.save()
        for functionality in app.functionalities.all():
          print("\t\tprocessing functionality %s" % functionality.path)
          menuLink = MenuLink(functionality = functionality, appMenu = appMenu)
          menuLink.save()